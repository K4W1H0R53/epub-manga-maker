...
def gene_xhtml(title):
    ...
    for path, dir_list, file_list in path_image:
        for index, file_name in enumerate(sorted(file_list)):
            ...
            </html>'''.format(src_width, src_height, file_name,title[0])
            f = open("./temp/OEBPS/Text/" + os.path.splitext(file_name)[0] + ".xhtml", mode="w", encoding="utf-8")
            f.write(page_html)
            f = open("./templates/blank_templates.xhtml", mode="r", encoding="utf-8")
            content = f.read()
            content = content.format(title[0])
            f = open("./temp/OEBPS/Text/blank.xhtml", mode= "w", encoding = "utf-8")
            f.write(content)
            f.close()
            spine_file = []
